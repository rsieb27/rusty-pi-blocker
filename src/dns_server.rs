...
impl DnsAdBlocker {
    pub fn new() -> Result<Self> {
        ...
        Ok(DnsAdBlocker { socket, blocklist })
    ...
    pub fn run(&self) -> Result<()> {
        ...
        loop {
            ...
            let list = self.blocklist.lock().map_err(|_| eyre!("Blocklist lock is poisoned"))?;
    
            if list.contains(&domain) {
                //log the block
                let _ = logger::log_blocked_domain(&domain, &src.ip().to_string()); //log domain
                //respond with nxdomain
                let nxdomain = Self::generate_nxdomain_response(&buf[..amt]);
                self.socket
                ...
                //forward to real dns
                Self::forward_dns_request(&buf[..amt], src, &self.socket)?;
            }
        }
    }
